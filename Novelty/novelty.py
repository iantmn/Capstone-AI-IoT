...
from novelty_detection import Anomaly_detection
...
def main():
    ...
    # p.pprint(datapd)
    nov = Anomaly_detection(r'../Model_Active_Learning/model_running_50.txt', r'../Data Gathering and Preprocessing/features_Walking_scaled.csv')
    print(nov.labels)
    # for i in range(len(nov.labels)):
    #     print(len(nov.closest_to_centres[i]), len(nov.possible_novelties[i]))
    nov.detect()
    # nov.find_centres()
    # datapd = pd.read_csv(r'../Data Gathering and Preprocessing/features_Walking_scaled.csv')
    # l = len(datapd)
