...
def main() -> None:
    ...
    empty_files(['features_skip_cleaning.txt', 'features_skip_cleaning_scaled.csv', 'processed_data_files.txt'])
    cln = Preprocessing('skip_cleaning')
    cln.windowing(r"csvfile_skip.csv", label='', data_from_gopro=True,
                start_offset=0.5, stop_offset=3, size=2, offset=0.2, epsilon=0.03, do_plot=False, do_scale = True)
